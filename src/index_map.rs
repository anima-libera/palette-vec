...
/// The IndexMap adapts itself to a few different sizes to squeeze memory usage even more.
///
...
impl NumberTypeForIndexMap for u16 {}
impl NumberTypeForIndexMap for u32 {}
impl NumberTypeForIndexMap for u64 {}

#[derive(Clone)]
struct IndexMapSized<N>
where
    N: NumberTypeForIndexMap,
{
...
pub enum BrokenInvariantInIndexMap {
    ...
    NotSorted,
    /// The index map has two entries with the same `index_in_key_vec`.
    ///
    /// It makes no sense, it maps some `index_in_key_vec`s to OPSKs (one maximum per index),
    /// it cannot have two entries with the same `index_in_key_vec`.
    TwoEntriesHaveSameIndexInKeyVec {
        index_in_key_vec: usize,
        index_in_map_a: usize,
        index_in_map_b: usize,
    },
}
...
impl IndexMap {
    ...
    pub(crate) fn check_invariants(&self) -> Result<(), BrokenInvariantInIndexMap> {
        match &self.inner {
            IndexMapEnum::U16(map_sized_u16) => map_sized_u16.check_invariants(),
            IndexMapEnum::U32(map_sized_u32) => map_sized_u32.check_invariants(),
            IndexMapEnum::U64(map_sized_u64) => map_sized_u64.check_invariants(),
        }
    }
}
    ...
    N: NumberTypeForIndexMap,
{
    ...
    /// shall leave `Self` in a valid state,
    /// if that does not happen then the method has a bug.
    fn check_invariants(&self) -> Result<(), BrokenInvariantInIndexMap> {
        if !self.vec.is_sorted_by_key(|entry| entry.index_in_key_vec) {
            return Err(BrokenInvariantInIndexMap::NotSorted);
        }

        // Check for duplicates in `index_in_key_vec`s.
        // Since we already checked that the vec is sorted by `index_in_key_vec`
        // then if there are any duplicates the duplicates are adjacent.
        for (index_in_map_of_first_entry, two_entries) in self.vec.windows(2).enumerate() {
            let entry_a = two_entries[0];
            let entry_b = two_entries[1];
            if entry_a.index_in_key_vec == entry_b.index_in_key_vec {
                return Err(BrokenInvariantInIndexMap::TwoEntriesHaveSameIndexInKeyVec {
                    index_in_key_vec: entry_a.index_in_key_vec.try_into().ok().unwrap(),
                    index_in_map_a: index_in_map_of_first_entry,
                    index_in_map_b: index_in_map_of_first_entry + 1,
                });
            }
        }
...
impl IndexMap {
    /// Creates an empty `IndexMap` (with the smallest entry size).
    ///
...
mod tests {
    ...
    fn access_empty_returns_none() {
        ...
        assert!(index_map.check_invariants().is_ok());
    }
